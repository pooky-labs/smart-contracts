    ...
    (err: Error | null, results?: string[]) => {
      ...
      if (results != undefined) {
        results.forEach((fullPath: string) => {
          ...
          asyncWriteFile(finalPath, abi);
        });

        console.log('Successfully extracted the contract ABIs');
      }
    },
...
main().catch((error) => {
  console.error('Failed to extract the contract ABIs', error);
  process.exitCode = 1;
});
