  ...
  describe("_beforeTokenTransfer", async () => {
    it("should revert if a player attempts to transfer tokens", async () => {
      const amount = parseEther(
        faker.datatype.number({ min: 100, max: 150 }).toString()
      );
      await expect(
        connect(POK, minter).mint(player1.address, amount)
      ).to.changeTokenBalance(POK, player1.address, amount);
      await expect(
        connect(POK, player1).transfer(player2.address, amount / 10n)
      )
