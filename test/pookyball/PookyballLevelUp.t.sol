...
import { PookyballMetadata, PookyballRarity } from "@/pookyball/IPookyball.sol";
...
contract PookyballLevelUpTest is BaseTest, PookyballSetup, LevelUpSetup {
  ...
  function test_levelUp_revertInsufficientPOK() public {
    ...
    vm.prank(user);
    vm.expectRevert(
      abi.encodeWithSelector(LevelUp.InsufficientPOK.selector, pricing.feePOK + pricing.gapPOK, 0)
    );
    levelUp.levelUp(
  ...
  function test_levelUp_revertTransferFailed() public {
    ...
    vm.prank(user);
    vm.expectRevert(
      abi.encodeWithSelector(ITreasury.TransferFailed.selector, address(invalid), value)
    );
