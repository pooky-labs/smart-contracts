...
describe('NonceRegistry', () => {
  ...
  describe('set', () => {
    it('should revert a non-operator tries to set a hash', async () => {
      await expect(connect(NonceRegistry, player1).set(hash, true)).to.be.revertedMissingRole(player1, OPERATOR);
    });
  ...
  describe('setBatch', () => {
    it('should revert a non-operator tries to set a hash', async () => {
      await expect(connect(NonceRegistry, player1).setBatch([hash], [true])).to.be.revertedMissingRole(
        player1,
        OPERATOR,
      );
    });
    ...
    it('should allow operator to set multiple hashes to true', async () => {
      const hashes = range(20).map(() => solidityPackedKeccak256(['string'], [faker.datatype.string()]));
