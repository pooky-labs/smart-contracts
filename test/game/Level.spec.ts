...
describe("Level", () => {
  ...
  describe("levelPXP", () => {
    it("should return coherent level PXP amount", async () => {
      ...
      expect(await Level.levelPXP(10)).to.approximately(
        parseEther("115"),
        delta
      ); // level 10 is 115 PXP
      expect(await Level.levelPXP(20)).to.approximately(
        parseEther("237"),
        delta
      ); // level 20 is 237 PXP
    });
  ...
  describe("levelPOK", () => {
    it("should return coherent level $POK", async () => {
      ...
      expect(await Level.levelPOK(2)).to.approximately(
        parseEther("5.2"),
        delta
      ); // level 2 is 5.2 $POK
      expect(await Level.levelPOK(10)).to.approximately(
        parseEther("9.2"),
        delta
      ); // level 10 is 9.2 $POK
      expect(await Level.levelPOK(20)).to.approximately(
        parseEther("19.0"),
        delta
      ); // level 20 is 19.0 $POK
    });
  });
  ...
  describe("levelPOKCost", () => {
    it("should match levelPOK if ball has enough PXP", async () => {
      ...
      expect(await Level.levelPOKCost(tokenId, 1)).to.eq(
        await Level.levelPOK(nextLevel)
      );
    });
    ...
    it("should allow $POK to cover missing PXP", async () => {
      ...
      expect(await Level.levelPOKCost(tokenId, 1)).to.gt(
        await Level.levelPOK(nextLevel)
      );
    });
  });
  ...
  describe("levelUp", async () => {
    ...
    it("should allow level up if Pookyball token has PXP and player owns enough $POK tokens", async () => {
      ...
      await expect(
        connect(Level, player1).levelUp(tokenId, 1)
      ).to.changeTokenBalance(POK, player1.address, -amountPOK);
