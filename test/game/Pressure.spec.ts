...
describe("Pressure", () => {
  ...
  describe("priceNAT", () => {
    ...
    for (const [current, amount, expected] of dataset) {
      it(`should compute cost for current=${current},amount=${amount}`, async () => {
        await expect(
          parseFloat(
            formatEther((await Pressure.priceNAT(current, amount)).toString())
          )
  ...
  describe("pricePOK", () => {
    ...
    for (const [current, amount, expected] of dataset) {
      it(`should compute cost for current=${current},amount=${amount}`, async () => {
        expect(
          parseFloat(formatEther(await Pressure.pricePOK(current, amount)))
        ).to.be.approximately(expected, 0.01);
      });
    }
  });
  ...
  describe("inflate", () => {
    ...
    describe("using native currency", () => {
      ...
      it("should revert if transaction value is insufficient", async () => {
        ...
        await expect(
          connect(Pressure, player1).inflate(tokenId, current, amount, {
            value,
          })
        )
