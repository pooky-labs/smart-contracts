...
task("getMinTierToBuy", "Gets minimum tier to mint ball").setAction(
  async (params, hre) => {
...
task("getMaxBallsPerUser", "Gets maximum balls per user").setAction(
  async (params, hre) => {
    await hre.run("set-hre");
    ...
    console.log(
      "Maximum balls per user is " + hre.ethers.utils.formatEther(maxBalls)
    );
  }
);

task("getMintsLeft", "Gets mints left of player").setAction(
  async (params, hre) => {
    await hre.run("set-hre");
    let [, , , , player] = await hre.ethers.getSigners();
    ...
    console.log(
      "Number of mints left for player is " +
        hre.ethers.utils.formatEther(mintLeft)
    );
  }
);
...
task("getAllMintTemplates", "Prints all mint templates").setAction(
  async (params, hre) => {
    await hre.run("set-hre");
...
task(
  "getPookyBallContract",
  "Gets PookyBall contract",
  async (taskArgs, hre) => {
    await hre.run("set-hre");
    ...
    console.log(
      "Pooky ball contract address in PookyMintEvent contract is " +
        pookyBallContractAddress
    );
    ...
    console.log(
      "Pooky ball contract address in PookyGame contract is " +
        pookyBallContractAddress
    );
  }
);
...
task(
  "getPookySigner",
  "Gets BE signer from PookyBall contract",
  async (taskArgs, hre) => {
    await hre.run("set-hre");
...
task(
  "getPOKToken",
  "Gets POK token address from PookyGame contract",
  async (taskArgs, hre) => {
    await hre.run("set-hre");
    ...
    console.log(
      "Pook Token address in PookyGame contract is " + pokTokenAddress
    );
  }
);
...
task(
  "getPookyBallContractUri",
  "Gets contract URI from PookyBall contract"
).setAction(async (params, hre) => {
  await hre.run("set-hre");
...
task("getPookyBallBaseUri", "Gets base URI from PookyBall contract").setAction(
  async (params, hre) => {
    await hre.run("set-hre");
